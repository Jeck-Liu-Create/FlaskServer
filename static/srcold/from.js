...
//返回结束日期
    ...
    function actionFormatter( value, row) {
        var result = "";
        // list = [ [0]ID , [1]轧辊号 ， [2]直径  ， [3]机架号 ，[4]轧辊类型 ，[5]轧辊位置 ，[6]轧辊材质 ， [7]凸度正负标记 ， [8]粗糙度， [9]操作侧轴承号 ， [10]驱动侧轴承号 ， [11]配对辊号 ， [12]锥形顶端直径 ， [13]锥形底端直径 ， [14]索引, [15]磨削原因， [16]探伤结果, [17]凸度值 ,[18]是否毛化标记 ]
        var list = [row.c_id,row.c_roll_number,row.c_diameter,row.c_frame_id,row.c_roll_type,row.c_roll_position,row.c_roll_material,row.c_crown_symbol,row.c_roughness,row.c_side_bearing,row.c_drive_bearing,row.c_pairing_roll,row.c_top_diameter,row.c_low_diameter,row.number,row.c_cause,row.c_result_detection,row.c_crown_value,row.c_woolification];
    ...
    function EditViewById(data){
        ...
        // $("#modal_c_woolification").val(row[18]);//是否毛化标记
        if (row[18]===null || row[18]==='')
        {
            console.log(typeof row[18] +  row[18] + "未毛化");
            $("#modal_c_woolification").val("0:未毛化");//未毛化
        }else {
            console.log(typeof row[18] +  row[18] + "(毛化");
            $("#modal_c_woolification").val(row[18]);//毛化
        }
    ...
    // $(function (){
    //     $('#send-btn').attr("style","display:block")
    // })
    $('#demo').click(function (){
        var data = "dead"
        // console.log($("#send-btn").attr("display"))
        // if ($("#send-btn").attr("display")==null)
        // {
        //     console.log('12')
        //     $('#send-btn').attr("style","display:block")
        // }
    ...
    $('#sava-edit-btn').click(function() {
        ...
        if (show === 'block') {
            ...
            var boolstr = (boolstr_c_diameter && boolstr_check_c_crown && boolstr_c_roughness && boolstr_c_side_bearing && boolstr_c_drive_bearing && boolstr_c_pairing_roll && boolstr_c_top_diameter && boolstr_c_low_diameter && boolstr_c_frame_id && boolstr_c_roll_type && boolstr_c_roll_position && boolstr_c_roll_material && boolstr_c_crown_symbol && boolstr_c_woolification)
            console.log(boolstr);
            if (boolstr) {
            ...
            if (boolstr_none) {
                ...
                var c_woolification_none = $('#modal_c_woolification').val();
                var data_none = {
                    data: JSON.stringify({
                        ...
                        'c_woolification_none': c_woolification_none,
                    })
                };
                            ...
                            $(function () {
                                ...
                                $('#tabledata').bootstrapTable('updateRow', {
                                    ...
                                    row: {
                                        ...
                                        c_woolification: c_woolification_none,
                                    }
                                })
                            })
                            $.message({
    ...
    $(function () {
        ...
        $("#modal_c_woolification").blur(check_c_woolification)     // 是否毛化标记
